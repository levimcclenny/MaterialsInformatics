...
def test_single_autoencoder():
    ...
    data = pd.read_csv("../../Classifiers/DictLearn/micrograph.csv")
    ...
    for img in sw_imgs:
        print(i)
        imgs.append(image.extract_patches_2d(img, (100,100), max_patches = 3000, random_state = 50))
        i = i+1
    ...
    for i in range(len(imgs)):
        imgs_reshape.append(np.reshape(imgs[i], (10000)))
        
    targets = np.array([0]*len(imgs))
    
    
    Xtrain, Xtest, ytrain, ytest = train_test_split(
        imgs_reshape, targets, test_size=0.1, random_state=42)
    ...
    _, D = Xtrain.shape
    autoencoder = AutoEncoder(D, 5000, 0)
    init_op = tf.global_variables_initializer()
    with tf.Session() as session:
        session.run(init_op)
        ...
        while not done:
            ...
            plt.subplot(1,2,2)
            plt.imshow(y.reshape(100,100), cmap='gray')
