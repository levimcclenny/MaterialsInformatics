...
InputName = '/Users/levimcclenny/Desktop/Materials Informatics/image generation/autoencoders/micrographs/micrograph1457.tif'
img_sample = cv2.imread(InputName)
img_sample = img_sample[0:400, 0:400]
img_height = 270
img_width  = 270
...
PatchSize = int(50)
OverlapWidth = int(15)
InitialThresConstant = float(78.0)
...
print("Progress : [%-20s] %d%% | PixelsCompleted: %d | ThresholdConstant: --.------" % ('='*int(pixelsCompleted*20/TotalPatches), (100*pixelsCompleted)/TotalPatches, pixelsCompleted))
#sys.stdout.flushsys.stdout.flush()
while GrowPatchLocation[0] + PatchSize < img_height:
    ...
    print('\r')
    print("Progress : [%-20s] %d%% | PixelsCompleted: %d | ThresholdConstant: %f" % ('='*int(pixelsCompleted*20/TotalPatches), (100*pixelsCompleted)/TotalPatches, pixelsCompleted, ThresholdConstant))
    #sys.stdout.flush()
...
plt.subplot(1,2,1)
plt.imshow(img_sample[0:250,0:250], cmap='gray')
plt.title('Original')

plt.subplot(1,2,2)
plt.imshow(img[0:250,0:250], cmap='gray')
plt.title('Reconstructed')
plt.savefig('compare_spher.png')